    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-8 animate-in fade-in pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex bg-slate-200/50 p-1 rounded-xl border border-slate-200 overflow-x-auto custom-scrollbar">
           ...
           <TabButton active={activeTab === 'chat'} onClick={() => setActiveTab('chat')} label="Mensagens" icon={MessageCircle} />
        </div>
      </div>
      ...
      {activeTab === 'classes' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           ...
           <div className="lg:col-span-2">
              {selectedClass ? (
                 <div className="bg-white rounded-[2.5rem] border shadow-sm overflow-hidden flex flex-col h-full min-h-[500px]">
                    <div className="p-8 bg-slate-50 border-b flex justify-between items-center">
                       ...
                       <div className="relative">
                          ...
                          <input 
                             ...
                             placeholder="ID ou E-mail do Aluno..." 
                             className="pl-9 pr-4 py-2 bg-white border rounded-lg text-xs w-64 focus:ring-2 focus:ring-accent-gold shadow-inner" 
                          />
                          {studentSearch.trim() && (
                             <div className="absolute top-full left-0 right-0 mt-2 bg-white border rounded-xl shadow-2xl z-20 max-h-60 overflow-y-auto border-slate-200">
                                {allUsers.filter(u => u.role === UserRole.STUDENT && (u.email.includes(studentSearch) || u.id.includes(studentSearch))).map(u => (
                                   <button 
                                     key={u.id} 
                                   ...
                                   >
                                      <div><p className="font-bold text-legal-900">{u.name}</p><p className="text-[10px] text-slate-400">{u.email}</p></div>
...
const TabButton = ({ active, onClick, label, icon: Icon }: any) => (
  <button onClick={onClick} className={`flex items-center gap-2 px-6 py-3 rounded-lg text-xs font-bold transition-all whitespace-nowrap shadow-sm border ${active ? 'bg-white text-legal-900 border-slate-200' : 'text-slate-500 hover:text-slate-800 border-transparent hover:bg-slate-100'}`}>
    <Icon size={16}/> {label}
  </button>
);
